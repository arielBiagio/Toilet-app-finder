    ...
    {activeView === 'explore' && <div className="screen explore-screen">
      ...
      <section ref={sheetRef} className={`results-sheet ${emergencyMode ? 'emergency-sheet' : ''} ${sheetPeek ? 'peek' : ''}`} aria-labelledby="results-title">
        ...
        {emergencyMode ? <>
          ...
          <div className="origin-card"><span className="origin-symbol"><Icon name={origin ? 'pin' : 'crosshair'} /></span><div><strong>{origin ? 'Ordenados por distancia' : 'Activa tu ubicación'}</strong><p>{origin ? `Origen ${origin.kind === 'device' ? 'del dispositivo' : 'manual'}.` : 'Para ver cuál queda más cerca.'}</p></div><button type="button" onClick={origin ? () => setManualFormOpen((open) => !open) : requestDeviceLocation} disabled={locationState === 'loading'}>{locationState === 'loading' ? '…' : origin ? (manualFormOpen ? 'Cerrar' : 'Cambiar') : 'Usar'}</button></div>
